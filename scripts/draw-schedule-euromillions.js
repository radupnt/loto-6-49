...
const DRAW_DAYS = new Set([2, 5]); // Marți, Vineri
const DRAW_HOUR = 20;
...
function formatDrawDisplay(year, month, day, weekday) {
  ...
  return `${dayName}, ${day} ${monthName} ${year}, ora ${DRAW_HOUR}:${String(DRAW_MINUTE).padStart(2, '0')} (UK)`;
}
...
function getNextDraw(from = new Date()) {
  ...
  for (let i = 0; i < 21; i++) {
    const parts = getLondonParts(cursor);
